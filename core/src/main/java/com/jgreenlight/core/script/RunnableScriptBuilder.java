...
public class RunnableScriptBuilder {
	@Autowired
	private StepAction stepCommand;

	public RunnableScript create(ScriptSource scriptSource) {
	...
	private List<RunnableStep> buildSteps(ScriptSource scriptSource,ScriptRunContext runContext,ParameterValuesResolver paramValuesResolver) {
		...
		for (ScriptStep scriptStep : scriptSource.getSteps()) {
			...
			RunnableStep execStep = new RunnableStep(runContext, stepName, hasOutput, stepOutputVarName, stepParams, paramValuesResolver, this.stepCommand);
